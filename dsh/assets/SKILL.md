...
Requires Python 3.10+ on the host. The deterministic tooling runs inside a
self-contained virtualenv so it never depends on the host's installed packages
and works even where the host refuses bare `pip install` (PEP 668). URL and
arXiv inputs need network access; local PDFs can be processed offline.

Resolve `SKILL_BASE` to the resource base directory the host supplied for this
skill — the directory containing this `SKILL.md` (the `skill` tool prints it as
"Base directory for this skill"). All relative references below
(`scripts/...`, `references/...`, `assets/...`) resolve against `SKILL_BASE`.
In the commands below, substitute that path for `<skill-base-directory>`.
Resolve `SKILL_PY` to the runtime virtualenv's Python binary printed by the
bootstrap step (see below).
...
### 0. Bootstrap the runtime and run preflight

Create (or refresh) the runtime virtualenv once per workspace:

```bash
SKILL_BASE="<skill-base-directory>"
python3 "${SKILL_BASE}/bootstrap.py" --dir ".work/venv"
```

The script is idempotent: it creates `.work/venv`, installs `pypdf`,
`requests`, and `jsonschema` inside it, and prints the virtualenv's Python
binary. Use that Python for every skill command below, for example on POSIX:

```bash
SKILL_PY=".work/venv/bin/python"
```

Then run preflight:

```bash
"${SKILL_PY}" "${SKILL_BASE}/scripts/preflight.py" \
...
automatically block the run. Re-run the bootstrap step if preflight reports a
missing dependency. See
...
```bash
"${SKILL_PY}" "${SKILL_BASE}/scripts/prepare_source.py" \
  "<INPUT>" --output-dir ".work/<RUN_ID>"
```
...
evidence. Text extraction alone is insufficient for visual evidence. When key
evidence is visual, render the source PDF's pages to PNG and view them with the
host's image tool:

```bash
"${SKILL_PY}" "${SKILL_BASE}/render_page.py" \
  --pdf ".work/<RUN_ID>/source.pdf" --page 4 --page 12 \
  --output-dir ".work/<RUN_ID>/figures"
```

If the render fails because `pymupdf` is missing from the runtime venv, install
it once and re-run:

```bash
"${SKILL_PY}" -m pip install pymupdf
```

Then view each rendered PNG with the `read_image` tool. If the host model does
not support image input, do not claim a visual inspection: read the figure
captions and in-text descriptions instead, keep `visuals_inspected` limited to
what you actually inspected, and record the limitation in
`analysis_boundary.extraction_warnings`.
...
```bash
"${SKILL_PY}" "${SKILL_BASE}/scripts/validate_artifacts.py" \
  --analysis outputs/<ID>/analysis.json \
  --manifest .work/<RUN_ID>/source-manifest.json \
  --run outputs/<ID>/run.json

"${SKILL_PY}" "${SKILL_BASE}/scripts/render_report.py" \
  --analysis outputs/<ID>/analysis.json \
  --output outputs/<ID>/report.html

"${SKILL_PY}" "${SKILL_BASE}/scripts/export_learning_dataset.py" \
  --analysis outputs/<ID>/analysis.json \
